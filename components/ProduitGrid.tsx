...
const ProduitGrid: React.FC<ProduitGridProps> = ({
  ...
  const sortedRows = rows
    .slice()
    .sort((a, b) =>
      (a.Produit_nom?.toString().toLowerCase() || "").localeCompare(
        b.Produit_nom?.toString().toLowerCase() || ""
      )
    );

  const getItemsPerPage = () => {
    if (typeof window === "undefined") return 4;
    if (window.innerWidth >= 1280) return 8; // xl → 4 colonnes × 2 lignes
    if (window.innerWidth >= 1024) return 8; // lg → 3 colonnes × 2 lignes
    return 4; // mobile / sm → 4 items
  };
                  ...
                  <button
                    ...
                    className={`px-3 py-1 border rounded cursor-pointer ${
                      currentPage === page
                        ...
                        : "bg-gray-200 text-gray-700 hover:bg-gray-300 hover:text-blue-600"
                    }`}
                  >
                    {page}
                  </button>
