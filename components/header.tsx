...
export function Header() {
  ...
  const handleClick = async () => {
    setLoading(true);
    try {
      await handleAddAchatFromNotif(); // garde ta fonction existante
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLogout = () => {
    localStorage.clear(); // Supprime le token ou infos de session
  ...
  const handlePaiement = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/paiement/lancer/`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
            ...
            {isClient ? (
              <>
                ...
                <button
                  ...
                  className="relative flex items-center cursor-pointer justify-center w-10 h-10 rounded-full bg-blue-100 hover:bg-blue-200 transition"
                >
                  <Bell className="w-5 h-5 text-blue-600" />
                  {notifications.filter((n) => !n.vue_client).length > 0 && (
                    <span className="absolute -top-1 -right-1 cursor-pointer bg-red-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
      ...
      {notifOpen && (
        <div
          ref={notifRef}
          className="absolute right-0 mt-2 w-80 bg-white shadow-lg rounded-lg border p-4 z-50"
        >
          ...
          {notifications.length === 0 ? (
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto cursor-pointer">
              {notifications.map((notif) => (
                <div
                  key={notif.id}
                  className={`p-2 rounded transition ${
                    !notif.vue_client ? "bg-blue-100 font-medium" : "bg-gray-30"
                  }`}
                >
                  ...
                  {notif.mode_reception === "magasin" ? (
                    <button
                      className="mt-2 w-full bg-blue-200 text-white py-1 rounded hover:bg-blue-200"
                      onClick={async () => {
                        if (!notif.vue_client) {
                          try {
                            await fetch(
                    ...
                    <button
                      className="mt-2 w-full bg-blue-200 text-white py-1 cursor-pointer rounded hover:bg-blue-200"
                      onClick={async () => {
                        if (!notif.vue_client) {
                          try {
                            await fetch(
      ...
      {selectedNotif && !selectedNotif.vue_client && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50 p-4">
          <div className="bg-white rounded-lg p-8 w-full max-w-[80vw] max-h-[98vh] overflow-y-auto shadow-lg relative">
            <button
            ...
            <h2 className="text-xl font-bold mb-4">Livraison à domicile</h2>

            {/* Infos client */}
            <div className="mb-4">
              <p>
                <span className="font-semibold">Client :</span>{" "}
                {selectedNotif.client.nom} {selectedNotif.client.prenom}
              </p>
              <p>
            ...
            <button
              className="mt-4 w-full bg-green-600 text-white py-2 rounded cursor-pointer hover:bg-green-700 disabled:bg-gray-400"
              onClick={handleClick}
              disabled={loading}
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        /> */}
                        <img
                          src={item.Produit_photo_url} // ⚡ URL publique Supabase
                          alt={item.Produit_nom}
                          className="w-12 h-12 object-cover rounded"
                        />
                        <div>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setCartCount(0);
                  setCartOpen(false);
                  toast.success("La commande a été envoyée avec succès");
                } catch (error) {
                  console.error(error);
