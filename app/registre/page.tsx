    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          let shortAddress = "";
          if (data.address) {
            const { road, neighbourhood, suburb, city, state, country } =
              data.address;
            shortAddress = [road, neighbourhood, suburb, city, state, country]
              .filter(Boolean)
              .join(", ");
          }
          ...
          setForm((prev) => ({
            ...
            latitude: lat,
            longitude: lon,
            adresse: shortAddress || prev.adresse,
          }));
        } catch (err) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-red-400/40 to-blue-600/90 p-4">
      <div className="flex flex-col md:flex-row w-full max-w-5xl bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="w-full md:w-1/2 p-4 lg:p-6 h-full">
            ...
            <div className="flex justify-between mt-2">
              {step > 1 && (
                <button
                  ...
                  className="px-4 py-2 text-white bg-gray-600 rounded cursor-pointer hover:bg-gray-700"
                >
                  ⬅ Précédent
                </button>
              )}
              {step < 3 ? (
                <button
                  type="button"
                  onClick={() => setStep(step + 1)}
                  className="ml-auto px-4 py-2 text-white bg-blue-600 rounded cursor-pointer hover:bg-blue-700"
                >
                  ➡ Suivant
                </button>
              ) : (
                <button
                  type="submit"
                  disabled={isLoading}
                  className="ml-auto px-4 py-2 text-white bg-blue-600 cursor-pointer rounded hover:bg-blue-700"
                >
